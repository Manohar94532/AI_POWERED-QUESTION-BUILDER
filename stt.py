...
def trainer_dashboard():
    ...
    with st.sidebar:
        ...
        selected = option_menu(
            ...
            icons=["upload", "question-circle", "eye", "check-circle",
                ...
                if qb_details:
                    ...
                    if file_format == "pdf":
                        ...
                        with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as temp_file:
                            ...
                            # Download button
                            st.download_button(label='Download PDF', data=temp_file.read(
                            ), file_name=f'questions_{qb_id_str}.pdf', mime='application/pdf')
                        ...
                        st.download_button(label='Download DOCX', data=buffer.getvalue(
                        ), file_name=f'questions_{qb_id_str}.docx', mime='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
                        ...
                        # Height of each textbox (0.5 inch)
                        height = Inches(0.5)
                        ...
                        for question in questions:
                            ...
                            # Move down for the next textbox (0.1 inch gap)
                        ...
                        st.download_button(label='Download PPTX', data=buffer.getvalue(
                        ), file_name=f'questions_{qb_id_str}.pptx', mime='application/vnd.openxmlformats-officedocument.presentationml.presentation')
                        ...
                        st.download_button(label='Download CSV', data=buffer.getvalue(
                        ), file_name=f'questions_{qb_id_str}.csv', mime='text/csv')
...
def generate_personalized_learning_plan_document(username):
    ...
    try:
        ...
        if learning_plan_doc:
            # Assuming learning_plan stores qb_id
            qb_id = learning_plan_doc.get('question_bank_id')
            if qb_id:
                learning_plan = prepare_learning_plan(qb_id, username)
                if learning_plan:
                    doc = docx.Document()
                    doc.add_paragraph(
                        f"Technology: {learning_plan['technology']}")
                    doc.add_paragraph(
                        f"Start Date: {learning_plan['start_date']}")
                    doc.add_paragraph(f"End Date: {learning_plan['end_date']}")
                    doc.add_paragraph(f"Status: {learning_plan['status']}")
                    doc.add_paragraph(
                        f"Estimated Time: {learning_plan['estimated_time']} hours")
...
def update_learning_plan_status(qb_id, username, new_status):
    ...
    try:
        ...
        qb_info = db.question_banks.find_one(
            {"_id": qb_id_obj}, {"technology": 1})
        ...
        if qb_info and 'technology' in qb_info:
            ...
            result = db.learning_plans.update_one(
                # Use qb_id for unique identification
...
def employee_dashboard(username):
    ...
    # Pass the username here  # Pass the username here
    notifications = get_notifications("employee", username)

    if 'chat_history' not in st.session_state:
        st.session_state.chat_history = []  # Initialize chat history
    user_id = st.session_state.get('user', {}).get(
        '_id', "unique_user_id")  # Use MongoDB _id as user_id
    ...
    with st.sidebar:
        ...
        selected_tab = option_menu(
            ...
            options=["Your Learning Plan",
                     ...
                     "Chatbot",
                     "Feedback"],  # required
            icons=["book", "pencil", "check-circle", "bar-chart",
                   "file-earmark-text", "chat", "folder", "file-earmark-text"],  # optional
            menu_icon="cast",  # optional
            default_index=0,  # optional
            orientation="vertical",
        )
    ...
    if selected_tab == "Your Learning Plan":
        ...
        if question_banks:
            ...
            if selected_qb:
                qb_id_str, _ = selected_qb
                # Convert to ObjectId for functions that expect it
                qb_id = ObjectId(qb_id_str)
                learning_plan = prepare_learning_plan(qb_id, username)
                if learning_plan:
                    st.write(
                        f"Technology: {', '.join(learning_plan['technology']) if isinstance(learning_plan['technology'], list) else learning_plan['technology']}")
                    st.write(f"Start Date: {learning_plan['start_date']}")
                    st.write(f"End Date: {learning_plan['end_date']}")
                    st.write(
                        f"Estimated Time to Complete: {learning_plan['estimated_time']} hours")

                    # Update status feature
                    new_status = st.selectbox("Update Status", ["In Progress", "Completed"], index=0 if learning_plan.get(
                        'status') == "In Progress" else 1)  # Set default based on current status
                    ...
                    if st.button("Generate Next Learning Plan"):
                        ...
                        if next_qb_id:
                            next_learning_plan = prepare_learning_plan(
                                next_qb_id, username)
                            if next_learning_plan:
                                st.write(
                                    f"Next Technology: {', '.join(next_learning_plan['technology']) if isinstance(next_learning_plan['technology'], list) else next_learning_plan['technology']}")
                                st.write(
                                    f"Next Start Date: {next_learning_plan['start_date']}")
                                st.write(
                                    f"Next End Date: {next_learning_plan['end_date']}")
                                st.write(
                                    f"Next Status: {next_learning_plan['status']}")
                                st.write(
                                    f"Next Estimated Time to Complete: {next_learning_plan['estimated_time']} hours")
                            else:
                                st.error(
                                    "Failed to generate next learning plan")
                        else:
        ...
        if question_banks:
            ...
            if selected_qb:
                ...
                if qb_details:
                    # Display metadata in columns
                    # Removed col4 for updated timestamp as it's not directly displayed
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        st.info(f"**ID:** {qb_details['_id']}")
                    with col2:
                    ...
                    if questions:
                        ...
                        st.warning(
                            "No questions available in this question bank.")
                            ...
                            for question in questions:
                                try:
                                    ...
                                    translated = GoogleTranslator(
                                        source='auto', target=language_code).translate(question)
                                    translated_questions.append(translated)
                                except Exception as e:
                                    st.error(f"Translation error: {e}")
                                    translated_questions.append(
                                        f"Error translating: {question}")
                ...
                if qb_details:
                    ...
                    for i, question in enumerate(questions):
                        ...
                        if question_type == "multiple-choice":
                            ...
                            # Default if type is unknown or missing
                            user_answers.append("")
                    ...
                    if st.button("Submit"):
                        ...
                        for i, user_answer in enumerate(user_answers):
                            if i < len(correct_answers) and user_answer.strip().lower() == correct_answers[i].strip().lower():
                                ...
                                st.error(
                                    f"Q{i+1}: Incorrect. Correct answer: {correct_answers[i].strip()}")
                            total_questions_answered += 1

                        # Display final score in a more prominent way
                        st.markdown(
                            f"### Final Score: {score}/{total_questions_answered}")
                        save_assessment_result(
...
def prepare_learning_plan(qb_id, username):
    ...
    try:
        ...
        if qb_details:
            ...
            start_date = (completed_date + timedelta(days=1)
                          ).strftime('%Y-%m-%d')
            ...
            estimated_end_date = calculate_estimated_end_date(
                topics, estimated_time, start_date)
